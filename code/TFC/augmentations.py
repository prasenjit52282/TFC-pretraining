...
def remove_frequency(x, pertub_ratio=0.0):
    mask = torch.rand(x.shape,dtype=torch.float32,device='cuda') > pertub_ratio # maskout_ratio are False
...
def add_frequency(x, pertub_ratio=0.0):
    mask = torch.rand(x.shape,dtype=torch.float32,device='cuda') > (1-pertub_ratio) # only pertub_ratio of all values are True
